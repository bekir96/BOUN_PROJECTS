    ...
    def broadcast_network(self):
        while True:
            for i in range(1,255):
                __target_ip__ = self.__network__ + "." + str(i)
                if self.__ip__ != __target_ip__:
                    tmp_username = "[" + self.__username__ 
                    tmp_ip = " " + self.__ip__
                    __packet__ = ','.join([tmp_username, tmp_ip, " announce]"])
                    self.write_file("broadcast_log.txt", __packet__)
                    start_new_thread(self.send_packet, (__target_ip__, self.__port__, __packet__))
                else:
                    continue
            time.sleep(60)    
...
chat = PythonChat(__username__, 5000)
