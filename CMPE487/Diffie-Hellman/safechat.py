...
class PRIME_KEY(object):
    ...
    def generate_prime_number(length=8):
        ...
        while not PRIME_KEY.is_prime(p, 128):
            p = PRIME_KEY.generate_prime_candidate(length)
        return p
        

class DH_Endpoint(object):
    def __init__(self, private_key, public_key1 = PRIME_KEY.generate_prime_number(), public_key2 = PRIME_KEY.generate_prime_number()):
    ...
    def encrypt_message(self, message):
        encoded_msg = ""
        key = str(self.full_key)
        encoded_msg = pyDes.triple_des(key.ljust(24)).encrypt(message, padmode=2)
        return encoded_msg
    
    def decrypt_message(self, encrypted_message):
        recovered_msg = ""
        key = str(self.full_key)
        recovered_msg = pyDes.triple_des(key.ljust(24)).decrypt(encrypted_message, padmode=2)
...
class PythonChat :
    ...
    def __init__(self, __username__, __port__, __private_key__ = PRIME_KEY.generate_prime_number()):
        ...
        self.__private_key__ = __private_key__
        self.__diffie_hellman__ = DH_Endpoint(self.__private_key__)
    ...
    def send_packet(self, __ip__, __port__, __packet__):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                ...
                s.send(__packet__.encode('utf-8'))
                s.close()
        except:
            pass
        ...
        while True:
            ...
            while __control__:
                with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                    ...
                    try:
                        s.sendto(__packet__.encode('utf-8'), ('<broadcast>', self.__port__))
                        __control__-=1
                    except Exception:
                        continue
                    s.close()
    ...
    def what_to_do(self):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            while True:
                ...
                if not data:
                    break
                __request__ = str(data.decode('utf-8'))

                try:
                    __sender_ip__ = None
                    if __request__[-1] != ']':
                        __sender_name__, __sender_ip__, __sender_type__, __message_packet__ = __request__[1:].split(",")
                        if __sender_name__ not in self.__all_users__:
                                continue
                        else:
                            if self.__diffie_hellman__.full_key == None:
                                break
                            else:
                                self.green_flag("[!]", __sender_name__, "has sent new message.")
                                self.__all_users__[__sender_name__][2]+=1
                                self.__total_unread__+=1
                                __message_packet__ = __sender_name__ + ": " + self.__diffie_hellman__.decrypt_message(bytes.fromhex(__message_packet__)) + "\n"
                                ...
                                self.__diffie_hellman__.full_key = None
                    else :
                        ...
                        if __sender_type__ == "message":
                            if __sender_name__ not in self.__all_users__:
                                ...
                                if self.__diffie_hellman__.full_key == None:
                                    ...
                                    self.__all_users__[__sender_name__][2]+=1
                                    self.__total_unread__+=1
                                    __message_packet__ = __sender_name__ + ": " + self.__diffie_hellman__.decrypt_message(str(__message_packet__).strip()) + "\n"
                                    if __sender_name__ not in self.__store_messages__ :
                                        self.__store_messages__[__sender_name__] = [__message_packet__]
                                    else :
                                        self.__store_messages__[__sender_name__].append(__message_packet__)
                        ...
                        elif __sender_type__ == "PARTIAL_KEY_TYPE_1":
                            self.__diffie_hellman__ = DH_Endpoint(self.__private_key__, int(__message_packet__[0]),int(__message_packet__[1]))
                            self.__diffie_hellman__.generate_full_key(int(__message_packet__[2]))
                            tmp_username = "[" + self.__username__ 
                            tmp_ip = " " + self.__ip__
                            __packet__ = ','.join([tmp_username, tmp_ip, " PARTIAL_KEY_TYPE_2", str(self.__diffie_hellman__.generate_partial_key())])
                            __packet__ = __packet__ + "]"
                            start_new_thread(self.send_packet, (__sender_ip__, self.__port__, __packet__))

                        elif __sender_type__ == "PARTIAL_KEY_TYPE_2":
                            self.__diffie_hellman__.generate_full_key(int(__message_packet__[0]))
    ...
    def send_message_packet(self, __target_user_name__, __target_user_ip__):
        ...
        if __message__ != "" :
            ...
            tmp_username = "[" + self.__username__ 
            tmp_ip = " " + self.__ip__
            g = PRIME_KEY.generate_prime_number()
            p = PRIME_KEY.generate_prime_number()
            while g == p:
                g = PRIME_KEY.generate_prime_number()
                p = PRIME_KEY.generate_prime_number()
            self.__diffie_hellman__ = DH_Endpoint(self.__private_key__, g, p)
            self.__diffie_hellman__.full_key == None
            __packet__ = ','.join([tmp_username, tmp_ip, " PARTIAL_KEY_TYPE_1", str(self.__diffie_hellman__.public_key1),str(self.__diffie_hellman__.public_key2),str(self.__diffie_hellman__.generate_partial_key())])
            __packet__ = __packet__ + "]"
            start_new_thread(self.send_packet, (__target_user_ip__, self.__port__, __packet__))
            
            while self.__diffie_hellman__.full_key == None:
                continue

            __message__ = self.__diffie_hellman__.encrypt_message(__message__).hex() 
