...
'''
    function levenshtein_distance takes two input string,(s,t) and calculates Levenshtein 
    distance and returns it, the corresponding edit table and the sequence of operations 
...
def levenshtein_distance(s, t):
    ...
    '''
        ...
        strings are not equal, we put a cost as 1, and then fill our 'd array' again.
    '''
...
def backtracing(d, m, n, s, t, operations):
    ...
    if (d[m-1][n-1] <= d[m-1][n]) & (d[m-1][n-1] <= d[m][n-1]) & (d[m-1][n-1] <= d[m][n]):  # Diagonal check
    ...
    elif (d[m][n-1] <= d[m-1][n-1]) & (d[m][n-1] <= d[m-1][n]) & (d[m][n-1] <= d[m][n]):    # Left check
    ...
    elif (d[m-1][n] <= d[m-1][n-1]) & (d[m-1][n] <= d[m][n-1]) & (d[m-1][n] <= d[m][n]):    # Up check
...
if __name__=="__main__":
    ...
    string_1 = sys.argv[1]
    string_2 = sys.argv[2]
    distance, d, operations = levenshtein_distance(string_1, string_2)  
    print("(a) The Levenshtein edit distance between the two input string: {}\n".format(distance))
